...
def test_gold_feature_creation():
    ...
    # Other date-specific tables    logistic_file = os.path.join(silver_dir, "order_logistics", f"silver_olist_order_logistics_{formatted_date}.parquet")
    ...
    history_file = os.path.join(silver_dir, "delivery_history", f"silver_olist_delivery_history_{formatted_date}.parquet")
    history_df = pd.read_parquet(history_file)
    print(f"Delivery history: {history_df.shape[0]} rows")
    
    seller_file = os.path.join(silver_dir, "seller_performance", f"silver_olist_seller_performance_{formatted_date}.parquet")
    seller_perform_df = pd.read_parquet(seller_file)
    print(f"Seller performance: {seller_perform_df.shape[0]} rows")
    
    concentration_file = os.path.join(silver_dir, "concentration", f"silver_olist_concentration_{formatted_date}.parquet")
